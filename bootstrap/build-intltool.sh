...
local_clean_source() {
    echo "build-intltool.sh cleaning $BUILD_DIR"
    rm -fr $BUILD_DIR
}
...
install_sources() {
    ...
    # get intltool
    curl -f -O http://ftp.gnome.org/pub/GNOME/sources/intltool/0.40/intltool-$INTLTOOL_VERSION.tar.bz2
    bunzip2 -c intltool-$INTLTOOL_VERSION.tar.bz2 | tar xv
...
build() {
    ...
    ./configure --prefix=$PREFIX && make && make install || die "$0 -- Could not build intltool."
    popd > /dev/null
