...
GNOME_COMMON_VERSION=3.7.4

BUILD_DIR=gnome-common
SCRIPT_DIR=../engine
: ${PREFIX:=~/.openwebrtc}
...
local_clean_source() {
    echo "build-gnome-common.sh cleaning $BUILD_DIR"
    rm -fr $BUILD_DIR
}
...
install_sources() {
    ...
    # get gnome-common
    git clone git://git.gnome.org/gnome-common
...
build() {
    ...
    # build gnome-common
    pushd gnome-common
    git checkout $GNOME_COMMON_VERSION
    ./autogen.sh --prefix=$PREFIX && make && make install || die "$0 -- Could not build gnome-common."
    popd > /dev/null
