...
build() {
    ...
    (
        ...
        export CFLAGS="$CFLAGS $GST_CFLAGS $GST_PLUGINS_BASE_CFLAGS $XML_CFLAGS $FFI_CFLAGS -DGST_PLUGIN_BUILD_STATIC"
        export LDFLAGS="$LDFLAGS $XML_LIBS"
        export CXXFLAGS="$CFLAGS $PLATFORM_CXXFLAGS"
        export OBJCFLAGS="$CFLAGS -fno-objc-arc"
        export LIBS="-lxml2 $GST_BASE_LIBS $GST_PLUGINS_BASE_LIBS $GIO_LIBS $ORC_LIBS $GST_LIBS $FFI_LIBS $LIBS"
